package com.example.user.timecircle.views
...
import com.example.user.timecircle.CIRCLE_NUM
import com.example.user.timecircle.CircleView
import com.example.user.timecircle.R
import com.example.user.timecircle.UNIT_ANGLE
...
class TimeCircleLayout @JvmOverloads constructor(context: Context, attrs: AttributeSet? = null, defStyleAttr: Int = 0) : FrameLayout(context, attrs, defStyleAttr) {

    private var isZoomed = false
    private val centerX by lazy { getContext().dimen(R.dimen.time_circle_length) / 2 }
    private val centerY by lazy { getContext().dimen(R.dimen.time_circle_length) / 2 }
    ...
    private fun zoomIn() {
        if (!isZoomed) {
            originX = x
            animate().scaleX(SCALE2).scaleY(SCALE2).x(ZOOM_IN_X).y(ZOOM_IN_Y).setDuration(DURATION).start()
//            circleImageView.animate().scaleX(CIRCLE_IMAGE_SCALE2).scaleY(CIRCLE_IMAGE_SCALE2).setDuration(duration).start()
    ...
    fun zoomOut() {
        if (isZoomed) {
            initValues()
            animate().scaleX(SCALE1).scaleY(SCALE1).x(originX).y(ZOOM_OUT_Y).setDuration(DURATION).rotation(rotateAngle).start()
//            circleImageView.animate().scaleX(CIRCLE_IMAGE_SCALE1).scaleY(CIRCLE_IMAGE_SCALE1).setDuration(duration).start()
    ...
    private fun changeColorAndRotate(x: Float, y: Float) {
        ...
        animate().rotation(-rotateAngle).setDuration(300).start()
    }
